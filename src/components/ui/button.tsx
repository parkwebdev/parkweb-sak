  ...
  "inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-md text-xs font-medium transition-colors disabled:pointer-events-none disabled:opacity-50 [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        ...
        destructive:
          "bg-destructive text-destructive-foreground hover:bg-destructive/90 border border-transparent dark:shadow-[0_0_12px_hsl(350_75%_62%/0.4)] dark:hover:shadow-[0_0_16px_hsl(350_75%_62%/0.5)]",
        outline:
...
const Button = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant, size, asChild = false, loading = false, disabled, children, ...props }, ref) => {
    ...
    // Don't show loading spinner when asChild is used
    if (asChild) {
      return (
        <Comp
