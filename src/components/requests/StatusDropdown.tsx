...
export const StatusDropdown = ({ status, onStatusChange, variant = 'badge' }: StatusDropdownProps) => {
  ...
  return (
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <button 
          className={`inline-flex items-center rounded-full border px-2.5 py-0.5 text-xs font-semibold transition-colors cursor-pointer hover:opacity-80 ${getStatusColor(REQUEST_STATUSES[status], 'request')}`}
        >
          {REQUEST_STATUSES[status]}
          <ChevronDown size={12} className="ml-1" />
        </button>
