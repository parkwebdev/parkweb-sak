...
import { 
  ...
  WidgetBadge,
  WidgetDropdownWithState,
  WidgetDropdownMenuItem,
  WidgetDropdownMenuSeparator,
  WidgetDropdownMenuLabel,
  WidgetSeparator
} from './components';
import { 
  X, Send01, Minimize02, Home05, MessageChatCircle, HelpCircle, 
  ChevronRight, Zap, BookOpen01, Settings01, Microphone01, 
  Attachment01, Image03, FileCheck02, ThumbsUp, ThumbsDown 
...
// Simple toast replacement - inline notification
const showToast = (message: string, type: 'success' | 'error' = 'success') => {
  console.log(`[${type.toUpperCase()}]`, message);
  // Could implement a simple inline notification system here
};

// Simplified ChatBubbleIcon
const ChatBubbleIcon = ({ className }: { className?: string }) => (
  <MessageChatCircle className={className} />
);

export const ChatWidgetStandalone = ({ config: configProp, previewMode = false }: ChatWidgetStandaloneProps) => {
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      ...
      if (error.message?.includes('unique')) {
        showToast('You have already provided feedback for this article', 'error');
      } else {
  ...
  const widgetContent = (
    <>
      ...
      {isOpen ? (
          <WidgetCard className={isIframeMode ? "w-full h-full flex flex-col shadow-xl overflow-hidden border-0" : "w-[380px] max-h-[650px] flex flex-col shadow-xl overflow-hidden border-0"}>
            {/* Header */}
            {currentView === 'home' ? (
              <div className="relative h-[180px] overflow-hidden" style={getGradientStyle()}>
                ...
                <div className="absolute top-3 right-3 z-20 flex gap-1">
                  <WidgetDropdownWithState
                    trigger={
                      <WidgetButton variant="ghost" size="icon" className="text-white hover:bg-white/10 h-8 w-8">
                        <Settings01 className="h-4 w-4" />
                      </WidgetButton>
                    }
                  >
                    <WidgetDropdownMenuLabel>Chat Settings</WidgetDropdownMenuLabel>
                    <WidgetDropdownMenuSeparator />
                    <WidgetDropdownMenuItem
                  ...
                  <WidgetButton variant="ghost" size="icon" className="text-white hover:bg-white/10 h-8 w-8" onClick={() => setIsOpen(false)}>
                ...
                <div className="flex-1 flex flex-col">
                    ...
                    {!chatUser && (
                      <div className="flex items-start">
                        <div className="bg-muted rounded-lg p-3 w-full">
                          ...
                          >
                            <WidgetInput name="firstName" placeholder="First name" className="h-8 text-sm" required />
                            {formErrors.firstName && <p className="text-xs text-destructive">{formErrors.firstName}</p>}
                            <WidgetInput name="lastName" placeholder="Last name" className="h-8 text-sm" required />
                            {formErrors.lastName && <p className="text-xs text-destructive">{formErrors.lastName}</p>}
                            <WidgetInput name="email" type="email" placeholder="Email" className="h-8 text-sm" required />
                            {formErrors.email && <p className="text-xs text-destructive">{formErrors.email}</p>}
                            
                            {config.customFields.map(field => (
                              <div key={field.id}>
                                {field.fieldType === 'select' ? (
                                  <WidgetSelect name={field.id} required={field.required} className="h-8 text-sm">
                                  ...
                                  <WidgetTextarea name={field.id} placeholder={field.label} className="text-sm" required={field.required} />
                                ) : (
                                  <WidgetInput name={field.id} type={field.fieldType} placeholder={field.label} className="h-8 text-sm" required={field.required} />
                                )}
                              </div>
                            ))}
                            
                            <WidgetButton type="submit" size="sm" className="w-full h-8" style={{ backgroundColor: config.primaryColor }}>
                              Start Chat
                            </WidgetButton>
                          </form>
                        </div>
                  ...
                  {activeConversationId && (
                    <div className="p-4 border-t">
                      {isRecordingAudio && (
                        <div className="flex items-center justify-between mb-2 p-2 bg-red-500/10 rounded-lg">
                          <div className="flex items-center gap-2">
                            <div className="w-2 h-2 bg-red-500 rounded-full animate-pulse" />
                            <span className="text-sm">Recording... {recordingTime}s</span>
                          </div>
                          <WidgetButton size="sm" variant="destructive" onClick={stopAudioRecording}>
                            Stop
                          </WidgetButton>
                        </div>
                      )}
                        ...
                        <div className="flex gap-2 mb-2 overflow-x-auto">
                          {pendingFiles.map((pf, idx) => (
                            <div key={idx} className="relative flex-shrink-0">
                              {pf.preview ? (
                                <img src={pf.preview} alt="" className="w-16 h-16 object-cover rounded" />
                              ) : (
                        ...
                        {config.enableFileAttachments && (
                          <>
                            ...
                            <label htmlFor="file-upload">
                              <WidgetButton size="icon" variant="ghost" className="h-9 w-9" type="button">
                            ...
                            {filteredArticles.map((article) => (
                              ...
                              >
                                <div className="flex items-start justify-between gap-2">
                                  ...
                                  {article.category && (
                                    <WidgetBadge variant="secondary" className="text-xs flex-shrink-0">
                                      {article.category}
                                    </WidgetBadge>
                                  )}
                                </div>
                        ...
                        {!feedbackSubmitted ? (
                          <div className="space-y-3">
                            <div className="flex gap-2">
                              <WidgetButton
                                size="sm"
                                variant={articleFeedback === 'helpful' ? 'primary' : 'secondary'}
                                onClick={() => { 
                              ...
                              <WidgetButton
                                size="sm"
                                variant={articleFeedback === 'not_helpful' ? 'primary' : 'secondary'}
                                onClick={() => { 
  ...
  // Standard embedded mode with fixed positioning
  return (
