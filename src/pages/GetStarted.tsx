 ...
 */
function CompletedBadgeIcon({ className }: { className?: string }) {
  return (
...
/**
 * Circular progress indicator for incomplete state
 */
function ProgressCircle({ completedCount, totalCount }: { completedCount: number; totalCount: number }) {
...
export function GetStarted() {
  ...
  return (
    <>
      ...
      <main className={`flex-1 min-h-0 h-full overflow-y-auto bg-background flex flex-col ${allComplete ? 'justify-center' : ''}`}>
        <div className="max-w-5xl mx-auto w-full py-4 lg:py-8 flex-1 flex flex-col">
          {/* Header */}
