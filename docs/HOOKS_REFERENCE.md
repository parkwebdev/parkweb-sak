...
const {
  accountOwnerId,  // string | null - The account owner's user ID
  isTeamMember,    // boolean - Whether current user is a team member (not owner)
  loading,         // boolean - Loading state
} = useAccountOwnerId();
```

**Use Case**: Used by all data hooks to scope queries to the account owner's data, enabling team members to see the same data as the owner.

**File**: `src/hooks/useAccountOwnerId.ts`
