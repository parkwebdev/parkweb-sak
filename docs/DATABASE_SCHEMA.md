...
### `automations`
Visual workflow automation definitions for agents.

| Column | Type | Nullable | Default | Description |
|--------|------|----------|---------|-------------|
| `id` | uuid | No | `gen_random_uuid()` | Primary key |
| `agent_id` | uuid | No | - | FK to agents (CASCADE delete) |
| `user_id` | uuid | No | - | Account owner ID for RLS |
| `name` | text | No | - | Automation name |
| `description` | text | Yes | - | Optional description |
| `icon` | text | Yes | `'Zap'` | Icon name from UntitledUI |
| `color` | text | Yes | `'blue'` | Color theme |
| `status` | automation_status | No | `'draft'` | `'draft'` \| `'active'` \| `'paused'` \| `'error'` |
| `enabled` | boolean | No | `true` | Whether automation is enabled |
| `trigger_type` | automation_trigger_type | No | - | `'event'` \| `'schedule'` \| `'manual'` \| `'ai_tool'` |
| `trigger_config` | jsonb | No | `'{}'` | Trigger-specific configuration |
| `nodes` | jsonb | No | `'[]'` | React Flow nodes array |
| `edges` | jsonb | No | `'[]'` | React Flow edges array |
| `viewport` | jsonb | Yes | - | React Flow viewport state |
| `execution_count` | integer | No | `0` | Total execution count |
| `last_executed_at` | timestamptz | Yes | - | Last execution timestamp |
| `last_execution_status` | text | Yes | - | `'success'` \| `'error'` \| `null` |
| `created_at` | timestamptz | No | `now()` | Creation timestamp |
| `updated_at` | timestamptz | No | `now()` | Last update timestamp |
...
```typescript
// Event trigger
{ event: 'conversation.started' | 'lead.created' | 'booking.created' | ... }

// Schedule trigger
{ schedule: '0 9 * * MON-FRI', timezone: 'America/New_York' }

// Manual trigger
{ allowedRoles: ['admin', 'manager'] }

// AI Tool trigger
{ toolName: 'escalate_to_manager', description: 'Escalate urgent issues' }
```

**RLS Policies:**
- Users can create for accessible agents (`has_account_access(user_id)`)
- Users can view/update/delete accessible automations (`has_account_access(user_id)`)
...
|--------|------|----------|---------|-------------|
| `id` | uuid | No | `gen_random_uuid()` | Primary key |
| `automation_id` | uuid | No | - | FK to automations (CASCADE) |
| `trigger_type` | automation_trigger_type | No | - | How it was triggered |
| `trigger_data` | jsonb | Yes | - | Input data from trigger |
| `status` | text | No | `'pending'` | `'pending'` \| `'running'` \| `'completed'` \| `'failed'` |
| `error` | text | Yes | - | Error message if failed |
| `error_node_id` | text | Yes | - | Node ID that caused error |
| `nodes_executed` | jsonb | Yes | - | Per-node execution results |
| `variables` | jsonb | Yes | - | Variable state at end |
...
| `test_mode` | boolean | No | `false` | Whether this was a test run |
| `triggered_by` | uuid | Yes | - | User who triggered (if manual) |
| `lead_id` | uuid | Yes | - | Associated lead (if applicable) |
| `conversation_id` | uuid | Yes | - | Associated conversation (if applicable) |

**RLS Policies:**
- Inherits access from parent automation via `has_account_access()`
