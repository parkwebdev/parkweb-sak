    ...
    switch (action) {
      ...
      case 'sync': {
        ...
        if (!eventsResponse.ok) {
          ...
          console.error('[outlook-calendar-auth] Events sync failed:', errorText);
          return new Response(
        ...
        for (const event of eventsData.value || []) {
          ...
          const eventStatus = event.isCancelled ? 'cancelled' : 
        ...
        console.log(`[outlook-calendar-auth] Manual sync completed: ${eventsUpdated} events processed`);
      ...
      case 'refresh-webhook': {
        ...
        try {
          const webhookData = await createWebhookSubscription(account.access_token);

          if (webhookData) {
            await supabase
              .from('connected_accounts')
              .update({
                webhook_channel_id: webhookData.subscriptionId,
                webhook_expires_at: webhookData.expiresAt,
                sync_error: null,
              })
              .eq('id', accountId);

            console.log(`[outlook-calendar-auth] Webhook refreshed for account ${accountId}`);
