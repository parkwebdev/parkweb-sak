/**
 * PDF Table Component for Edge Functions
 * 
 * Ported from src/lib/pdf-components/PDFTable.tsx
 */
...
// @ts-ignore - npm import for Deno
import { format, parseISO, isValid } from 'npm:date-fns@3.6.0';
...
export function PDFTable({ columns, data, maxRows = 15 }: PDFTableProps) {
  ...
  return React.createElement(
    ...
    React.createElement(
      View,
      { style: tableStyles.headerRow, wrap: false },
      columns.map((col) =>
        React.createElement(
          Text,
          { key: col.key, style: getCellStyle(col, true) },
          col.header
        )
      )
    ),
    ...
    displayData.map((row, rowIndex) =>
      React.createElement(
        ...
        columns.map((col) =>
          React.createElement(
            Text,
            { key: col.key, style: getCellStyle(col) },
